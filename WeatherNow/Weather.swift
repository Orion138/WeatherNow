        ...
        Alamofire.request(.GET, url).responseJSON { response in
            ...
            if let dict = result.value as? Dictionary<String, AnyObject> {
                ...
                if let mainDetl = dict["main"] as? Dictionary<String, AnyObject> {
                    ...
                    if let tmpMax = mainDetl["temp_max"] as? Double {
                        self._tempMax = NSString(format: "%.0f", tmpMax) as String
                    }
                    
                    if let tmpMin = mainDetl["temp_min"] as? Double {
                        self._tempMin = NSString(format: "%.0f", tmpMin) as String
                    }
